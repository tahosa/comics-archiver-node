    ...
    return requestPromise(url)
      .then( html => {
        ...
        imgTag.each((i, elem) => {
          ...
          if (i === imgTag.length - 1 && this.instance.noteSelector) {
        ...
        const nextUrl = utils.normalizeUrl(url, nextTag.attr('href'));
        debug(`Attempting next with ${nextUrl}`);
        // Check to see if we should continue downloading
        if (nextTag.attr('href') && nextUrl !== url) {
          this.instance
            .update({ latest: url })
            .then( () => {
              promises.push(this.download(utils.normalizeUrl(url, nextUrl)));
            });
        }
